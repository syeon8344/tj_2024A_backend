...
function 제품등록(){
  ...
  // 2. 데이터를 가공( 객체화 )
  let 제품번호 = 0;
  제품목록.length == 0 ? 제품번호 = 1 : 제품번호 = 제품목록[제품목록.length-1].제품번호 + 1
  // 현재 제품목록의 마지막 제품번호의 +1 , 길이 1부터 / 인덱스 0부터 / 끝인덱스: 제품목록.length-1 
...
function 주문등록( 제품번호 ){ console.log('주문등록()'+제품번호 )
 ...
 // 1.
  ...
  let 주문번호 = 0;
  주문목록.length == 0 ? 주문번호 = 1 : 주문번호 = 주문목록[주문목록.length-1].주문번호 + 1
  // 마지막 주문번호에 +1 , 마지막주문번호 = 마지막인덱스의주문번호 = 배열.length-1
  let date = new Date();  
